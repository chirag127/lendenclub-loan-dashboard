...
function renderReasons() {
  ...
  const cards = [
    ...
    { tone: "bad", icon: "💸", title: "Why 12-month loans lose money even though they quote the highest rates",
      ...
      why: `A year gives a weak borrower plenty of time to stop paying, and the borrowers who do repay usually foreclose early — so you receive far less than the year of interest you were promised. The ${pct((INS.fee_schedule || {}).schedule ? (INS.fee_schedule.schedule["12"]) : null)} fee on every EMI's returned principal is the steepest of any tenure, and the capital stays locked for 12 months while 2-month money could have recycled ~6 times. It is the worst of both worlds.` },
    ...
    { tone: "warn", icon: "🪜", title: `Why your ~${pct(st.avg_interest_rate)} average contracted rate becomes ~${pct(aX)} net — the full ladder`,
      ...
      why: `Two costs are unavoidable in P2P lending and neither appears in the quoted rate: the platform fee takes its % of the principal every EMI returns, and credit losses are paid later out of principal. Only after both are subtracted do you get the return you can actually spend — about <b>${pct(aX)}/yr</b>.` },
    { tone: "bad", icon: "🕳️", title: `Why 'successful loans' (${pct(nX)}) and 'all defaults' (${pct(aX)}) differ by ~${defPp != null ? defPp.toFixed(0) : "—"} points`,
      ...
      why: `Every default is counted as your money going out with only actual receipts coming back, and a zero-recovery loan is a 100% loss. That is the ~${defPp != null ? defPp.toFixed(0) : "—"}-point gap between the two lines. The ${pct(nX)} figure excludes the NPA book entirely — treat it as an upper bound, not a promise.` },
    { tone: "warn", icon: "⏳", title: `Why the active book projects ~${pct(AX.portfolio_expected)} while you have realised ~${pct(aX)}`,
