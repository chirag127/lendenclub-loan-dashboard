...
Interactive **lending-decision dashboard** for a **LenDenClub (Lending Club India) manual lending report** — **51 curated charts (of 147 defined)** including 8 decision heatmaps, a fine-bucket **net-XIRR atlas** (50+ tenure × score buckets), **fee-model proof charts** and an **actual-vs-recommended allocation view**, plus gauge dials, plain-HTML tables (tenure × score risk matrix + NPA-by-year and vintage ledgers), KPIs, recommendation panels and a full loan register. Everything shown exists to answer one question: *which loans should I fund next — and which to avoid?* Hosted on GitHub Pages.

The curated set is **not fixed at 51** — more charts and other forms (gauges, tables, matrices) are added by listing their ids in one file (`assets/js/curation.js`).
...
![Charts](https://img.shields.io/badge/charts-51%20curated-brightgreen) ![Loans](https://img.shields.io/badge/loans-3%2C672-blue) ![Stack](https://img.shields.io/badge/stack-pure%20static%20html%2Bjs%2Becharts-orange) ![Audit](https://img.shields.io/badge/audit-34%2F44%20checks%20passed-brightgreen)
...
## Dashboard features — a decision flow, 51 curated charts in 9 sections (not a fixed cap)
...
- **Data-integrity audit bar** — every figure reconciled against the source report by `scripts/ldc/audit.py`; 44 checks (34 PASS · 10 notes · 0 FAIL) incl. the fee-model checks F1–F4 (fee = schedule % × principal returned, per pricing era) and the fresh-money allocation check X1, regenerated on every build
- **51 interactive charts** (Apache ECharts) in 9 sections + extra forms:
  ...
  - *What loans actually pay — net of fees & defaults* (10) — realized XIRR by tenure (successful vs **incl. all defaults**), simple-vs-XIRR explainer, the full rate ladder (sticker → fees → defaults → net), the total fee waterfall, **the fee increase (4/5-month loans now pay 3.0% where they paid 2.3–2.5%)**, the **net-XIRR pick heatmap (tenure × score)**, the **projected net-ROI heatmap (tenure × score)**, contracted-vs-collected interest, default-rate sensitivity, and the money-map bubble (projected ROI vs ₹ at risk)
  - *Where loans default — risk by tenure × score* (6 + **risk-matrix table**) — matured default rates by tenure, the NPA heatmap tenure × score, loss rate by tenure, share of book vs share of NPAs, interest vs NPA loss, risk-vs-return bubbles + **5 auto-computed lending guardrail cards** + a plain-HTML **tenure × score risk reference table** (every cell: loans, NPA %, matured default %, loss % — no cell hidden)
  ...
  - *The verdict — lend only these* (5 + 2 panels) — the **Highest-XIRR loan picks panel** (every tenure × score cell with ≥10 completed loans ranked by net XIRR incl. all defaults, tiered Core/Support/Avoid, per-₹1,000 split from `xirr_picks()`), allocation donut, successful-vs-defaults chart, **three new allocation charts** (`month_allocation()`: this month's money by tenure vs recommended, at the tenure × score cell level, and collapsed into verdict tiers — Sep-26 deployed ₹2.65L with only 56.8% in core cells), the **“Lend only these” verdict strip** and **17 plain-language reason cards** (every figure read live from the data globals: why 2–3 month wins, why 12-month loses, why 6-month below 750 is the NPA engine, the 68.0 → 53.5 → 23.1% ladder — with the NPA-timing correction that front-loads defaulted loans' receipts — how the fee actually works, and risk = time × borrower quality)
- **Tabs & chart density** — an "All views" tab plus one tab per section (and a **Full registry** tab exposing every non-curated definition); Compact / Standard / **Everything** density switch, so the page can show as few as ~33 or as many as 142 charts. Sections whose filtered slice has no loans render a friendly empty-state instead of zeros — whole-book sections (atlas, by-year, vintage) keep their full history regardless.
...
- **Sortable, searchable loan register** — all 3,672 loans (loan ID, order ID, dates, amount, status, rate, tenure, score, DPD, received, interest, P&L)

The remaining 96 chart definitions (seasonality, status treemaps, correlations, pure monthly actuals, the 38 diagnostic atlas maps, the fee-proof scatter…) stay in `assets/js/charts/*.js` but are **not rendered** — reachable via the Full-registry tab / Everything density. Nothing is hard-capped: the render set is curated in one place (`assets/js/curation.js`) — add a chart id to a group and it shows; add a new `addChart(...)` and it becomes available there too.

## Repo structure

```
├── index.html            # dashboard page (v19 — loads data globals + modular JS)
├── assets/
...
│       ├── curation.js   # ★ what renders: the 47 decision charts in 9 sections (+ Full-registry tab)
